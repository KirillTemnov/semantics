...
((exports, util) ->
  ...
  exports.personals = personals = ["вам", "вами", "вас", "вы", "его", "ей", "ему", "ею", "её", "им", "ими", "их", "мы", "нам", "нами", "нас", "него", "ней", "нему", "нею", "неё", "ним", "ними", "них", "нём", "он", "она", "оне", "они", "оно", "тебе", "тебя", "тобой", "тобою", "ты", "я"]
  ...
  exports.preFilter = (text, result) ->
    ...
    for s in result?.misc?.sentences || []
      reduce = s.toLowerCase().split(" ").filter( (wrd) -> /^[-а-яё]+$/.test wrd)
      for w in reduce
        if w in unions or w in personals or w in adverbs or w in prepositions
          reg_words.push w
        else
          words.push w
    result.ru =
